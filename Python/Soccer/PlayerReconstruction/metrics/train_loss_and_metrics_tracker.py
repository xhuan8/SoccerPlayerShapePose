...
class TrainingLossesAndMetricsTracker:
    ...
    def __init__(self, losses_to_track, metrics_to_track, img_wh, log_path,
                 load_logs=False, current_epoch=None, print_log = False):
        ...
        self.all_metrics_types = ['train_pves', 'val_pves',
                                  ...
                                  'train_silhouette_iou', 'val_silhouette_iou']
    ...
    def update_per_batch(self, split, loss, task_losses_dict, pred_dict_input, target_dict_input,
                         num_inputs_in_batch,
        ...
        pred_dict = {}
        target_dict = {}
        for key in pred_dict_input.keys():
            pred_dict[key] = pred_dict_input[key].cpu().detach().numpy()
            target_dict[key] = target_dict_input[key].cpu().detach().numpy()
        ...
        if 'joints2D_l2es' in self.metrics_to_track:
            ...
            #                                                 self.img_wh)
            #target_joints2D_coco = undo_keypoint_normalisation(pred_joints2D_coco,
            #                                                 self.img_wh)
        ...
        if 'silhouette_iou' in self.metrics_to_track:
            pred_silhouette = pred_dict['silhouette']
            target_silhouette = target_dict['silhouette']
            #silhouette_iou_batch = np.sum((pred_silhouette - target_silhouette)**2)  # (bsize, num_joints)
            silhouette_iou_batch = pred_silhouette * np.log(target_silhouette + 1e-6) + (1 - pred_silhouette) * np.log(1 - target_silhouette + 1e-6)
            self.loss_metric_sums[split + '_silhouette_iou'] += -np.sum(silhouette_iou_batch)  # scalar

    def update_per_epoch(self):
        if self.loss_metric_sums['train_num_samples'] != 0:
            self.history['train_losses'].append(self.loss_metric_sums['train_losses'] /
                                            self.loss_metric_sums['train_num_samples'])
        if self.loss_metric_sums['val_num_samples'] != 0:
        ...
        for loss_type in self.all_per_task_loss_types:
            ...
            if loss_on in self.losses_to_track:
                if self.loss_metric_sums[split + '_num_samples'] != 0:
        ...
        if self.print_log:
            ...
            print('Train Loss: {:.5f}, Val Loss: {:.5f}'.format(
                self.history['train_losses'][-1] if len(self.history['train_losses']) > 0 else 0,
                                                                self.history['val_losses'][-1]))
            for metric in self.metrics_to_track:
                print('Train {}: {:.5f}, Val {}: {:.5f}'.format(metric,
                    self.history['train_'+metric][-1] if len(self.history['train_'+metric]) > 0 else 0,
                                                                metric,
