...
class Mesh:
    ...
    def texcoords(self):
        if self._texcoords is None and self.mesh_info is not None:
            with open(self.mesh_info.texcoords[len('https://dl.fbaipublicfiles.com/densepose/meshes/'):], 'rb') as f:
                self._texcoords = torch.as_tensor(pickle.load(f), dtype=torch.float).to(self.device)
            #load_mesh_auxiliary_data(self.mesh_info.texcoords, self.device)
