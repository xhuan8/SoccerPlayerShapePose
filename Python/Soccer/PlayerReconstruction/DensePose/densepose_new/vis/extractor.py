...
from ..vis.bounding_box import BoundingBoxVisualizer, ScoredBoundingBoxVisualizer
from ..vis.densepose_outputs_vertex import DensePoseOutputsVertexVisualizer
from ..vis.densepose_results import DensePoseResultsVisualizer
...
def extract_boxes_xywh_from_instances(instances: Instances, select=None):
    if instances.has("pred_boxes"):
        ...
        boxes_xywh = boxes_xyxy.tensor.clone()
...
class DensePoseOutputsExtractor(object):
    ...
    def __call__(
        ...
        dpout = instances.pred_densepose
        boxes_xyxy = instances.pred_boxes
        boxes_xywh = extract_boxes_xywh_from_instances(instances, select=select)
