...
class HomoscedasticUncertaintyWeightedMultiTaskLoss(nn.Module):
    ...
    def __init__(self,
        ...
        if init_loss_weights is not None:
            # Initialise log variances using given init loss weights for
            # vertices, joints2D,
            # joints3D, shape, pose
            if 'verts' in init_loss_weights:
                init_verts_log_var = -np.log(init_loss_weights['verts'] + eps)
            if 'joints2D' in init_loss_weights:
                init_joints2D_log_var = -np.log(init_loss_weights['joints2D'] + eps)
            if 'joints3D' in init_loss_weights:
                init_joints3D_log_var = -np.log(init_loss_weights['joints3D'] + eps)
            if 'pose_params' in init_loss_weights:
                init_pose_params_log_var = -np.log(init_loss_weights['pose_params'] + eps)
            if 'shape_params' in init_loss_weights:
                init_shape_params_log_var = -np.log(init_loss_weights['shape_params'] + eps)
            if 'silhouette' in init_loss_weights:
                init_silhouette_log_var = -np.log(init_loss_weights['silhouette'] + eps)
        else:
        ...
        if 'silhouette' in losses_on:
            self.silhouette_log_var = nn.Parameter(torch.tensor(init_silhouette_log_var).float(),
                                                 requires_grad=False)
            self.silhouette_log_var.requires_grad = True
            self.silhouette_loss = nn.MSELoss(reduction=reduction)
    ...
    def forward(self, labels, outputs):
        ...
        if 'silhouette' in self.losses_on:
            #silhouette_loss = self.silhouette_loss(outputs['silhouette'], labels['silhouette'])
            #print(outputs['silhouette'].shape)
            #print(labels['silhouette'].shape)
            #cv2.imwrite('PlayerReconstruction/1.png', outputs['silhouette'].cpu().detach().numpy()[0]*255)
            #cv2.imwrite('PlayerReconstruction/2.png', labels['silhouette'].cpu().detach().numpy()[0]*255)
            #silhouette_loss = torch.sum((outputs['silhouette'] - labels['silhouette']) ** 2)
            silhouette_loss = torch.sum(outputs['silhouette'] * torch.log(labels['silhouette'] + 1e-6) + (1 - outputs['silhouette']) * torch.log(1 - labels['silhouette'] + 1e-6))
            total_loss += -silhouette_loss * torch.exp(-self.silhouette_log_var) + self.silhouette_log_var
            #print(silhouette_loss)
            #total_loss += silhouette_loss
            #print(total_loss)
