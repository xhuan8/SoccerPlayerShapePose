...
def convert_weak_perspective_to_camera_translation_torch(cam_wp, focal_length, resolution):
    x = cam_wp[:, 1]
    y = cam_wp[:, 2]
    t = cam_wp[:, 0]
    t = 2 * focal_length / (resolution * t + 1e-9)
    translation = torch.stack([x, y, t], dim=-1)
    return translation

def convert_camera_translation_to_weak_perspective_torch(translation, focal_length, resolution):
    x = translation[:, 0]
    y = translation[:, 1]
    s = translation[:, 2]
    s = 2 * focal_length / (resolution * s + 1e-9)
    cam_wp = torch.stack([s, x, y], dim=-1)
    return cam_wp
