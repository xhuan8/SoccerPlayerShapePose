...
def get_largest_centred_bounding_box(bboxes, orig_w, orig_h):
    ...
    bbox_found = False
    i = 0
    largest_centred_bbox_index = None
...
def predict_densepose(input_image, predictor):
    ...
    #print('largest: ' + str(largest_centred_bbox_index))
    if largest_centred_bbox_index is None:
        return None, None
...
def predict_iuv_image(input_image, predictor):
    orig_h, orig_w = input_image.shape[:2]
    outputs = predictor(input_image)["instances"]
    bboxes = outputs.pred_boxes.tensor.cpu()  # Multiple densepose predictions if there are multiple people in the image
    bboxes_XYWH = BoxMode.convert(bboxes, BoxMode.XYXY_ABS, BoxMode.XYWH_ABS)
    ...
    #iuv = iuv_arr.transpose((1,2,0))
    #cv2.imwrite("Data/1.png", iuv)
    ...
    #cv2.imwrite("Data/1.png", I_image)
    #cv2.imwrite("Data/2.png", U_image)
    #cv2.imwrite("Data/3.png", V_image)

    IUV_image = cv2.merge((I_image, U_image, V_image))
